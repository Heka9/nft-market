(() => {
    ...
    da.init();
    const collectionsButtons = document.querySelectorAll(".collections__button");
    console.log(collectionsButtons);
    const collectionsfilter = document.querySelector(".collections__filter");
    if (collectionsButtons.length) collectionsfilter.addEventListener("click", (function(e) {
        if (e.target.closest(".collections__button")) {
            collectionsButtons.forEach((collectionsButton => {
                collectionsButton.classList.remove("active");
            }));
            e.target.classList.contains("collections__button") ? e.target.classList.add("active") : e.target.parentElement.classList.add("active");
        }
    }));
    const itemsCollections = document.querySelectorAll(".item-collections");
    itemsCollections.forEach(((itemCollections, index) => {
        let collectionsNumber = itemCollections.querySelector(".item-collections__number");
        collectionsNumber.innerHTML = index + 1;
    }));
    window["FLS"] = false;
    isWebp();
    menuInit();
